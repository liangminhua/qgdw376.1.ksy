...
params:
  - id: num_digits
    type: u1
    doc: Number of digits in this BCD representation. Only values from 1 to 10 inclusive are supported.
  - id: bits_per_digit
    type: u1
                 ...
                 (digits[last_idx - 6] * 1000000 +
                  (num_digits < 8 ? 0 :
                   (digits[last_idx - 7] * 10000000 +
                    (num_digits < 9 ? 0 :
                     (digits[last_idx - 8] * 100000000 +
                      (num_digits < 10 ? 0 :
                       (digits[last_idx - 9] * 1000000000)
                      )
                     )
                    )
                   )
